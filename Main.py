...
eye_detector = EyeDetector()  # Add this line
...
while True:
    ...
    # Add eye detection
    eye_points = eye_detector.find_eyes(img)
    ...
    if len(lmList) != 0:
        ...
        if fingers == [0, 0, 0, 0, 0] and active == 0:
            ...
            mode = 'Cursor'
            active = 1
        elif fingers == [1, 1, 0, 0, 1] and active == 0:  # Add this condition here
            mode = 'Eye'
            active = 1
    ...
    if mode == 'Eye':
        ...
        if eye_points is not None:
            # Get eye centers
            left_eye = eye_points[33]  # Left eye center
            right_eye = eye_points[263]  # Right eye center
            
            # Calculate eye midpoint for cursor control
            eye_mid_x = (left_eye[0] + right_eye[0]) // 2
            eye_mid_y = (left_eye[1] + right_eye[1]) // 2
            
            # Move cursor based on eye position
            screenWidth, screenHeight = pyautogui.size()
            X = int(np.interp(eye_mid_x, [100, 540], [0, screenWidth - 1]))
            Y = int(np.interp(eye_mid_y, [50, 350], [0, screenHeight - 1]))
            ...
            # Detect blink for clicking
            left_eye_height = abs(eye_points[159][1] - eye_points[145][1])
            right_eye_height = abs(eye_points[386][1] - eye_points[374][1])
            
            if left_eye_height < 5 and right_eye_height < 5:  # Both eyes closed
                pyautogui.click()
                cv2.putText(img, "Click!", (200, 50), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 0, 255), 2)
            elif left_eye_height < 5:  # Left eye wink
                pyautogui.rightClick()
                cv2.putText(img, "Right Click!", (200, 50), cv2.FONT_HERSHEY_COMPLEX, 1, (255, 0, 0), 2)
                
            # Draw eye tracking visualization
            cv2.circle(img, (eye_mid_x, eye_mid_y), 5, (255, 0, 0), -1)
            cv2.rectangle(img, (100, 50), (540, 350), (0, 255, 0), 2)
            
        if fingers == [0, 0, 0, 0, 0]:  # Fist to exit eye mode
            active = 0
            mode = 'N'
